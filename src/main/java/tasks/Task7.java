...
public class Task7 {
  ...
  public static Set<String> vacancyNames(Collection<Company> companies) {
    ...
    // если рассматривать n - кол-во компаний m - максимальное число различных вакансий у каждой
    // по памяти получим O(nm) - (это будет до того как
    // преобразовали stream в set), после преобразования в сет, будет O(k) - k количество уникальных вакансий
    // логичнее взять максимальную границу O(nm)
